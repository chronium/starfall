...
Local preview and deterministic capture modes also present the exact ten ordered `Draft0StarterMonsterCatalog.FirstPlayable` assignments as generated box-creature fixtures. Cyan 1.0-metre light creatures and orange 1.5-metre heavy creatures face each camp entry anchor and use a gentle identity-phased hover. This is presentation-only diagnostic evidence: it adds no AI, health, combat, selected monster assets, or airborne semantics. Connected mode remains truthful and presents no monsters until world-owned monster snapshots replace these local fixtures.
