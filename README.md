...
Connected placeholders preserve authoritative identity, ground position, velocity, facing, collision radius, behavior, target and integer health. Hover, one observed attack-state lunge, return tint, health-decrease flash and retained-tombstone collapse are presentation only. The state stream does not expose individual repeated attack or hit timing, so exact combat audiovisual timing remains owned by later combat-result protocol work. No selected monster asset or airborne gameplay semantics are implied.
...
The `--validate-character-content` probe loads and validates the same runtime cook without initializing SDL; unknown client arguments fail with exit code 2. The interactive preview includes a development console alongside the read-only concern windows. Press `T` while the debug shell is visible to open its bottom-left input, enter the zero-argument `ping` command, and press Enter to submit and close. Escape cancels and closes input without exiting; Up and Down navigate command history. While input is closed, at most six recent lines remain over the game view for ten seconds and fade over the next two seconds without an opaque panel or hit testing. `F12` remains the master debug-shell toggle, and `T` does nothing while the shell is hidden, including an initially `--debug-ui-hidden` launch. Local preview accepts console input but reports that an authoritative World connection is required. Feature commands remain later task-owned work.
