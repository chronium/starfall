...
The `--validate-character-content` probe loads and validates the same runtime cook without initializing SDL; unknown client arguments fail with exit code 2. Debug-shell windows remain read-only until CLIENT-0031. The connected World already accepts the session-bound zero-argument `ping_world` development command from every admitted player, but the Client does not yet expose a typed or console frontend. Feature commands remain later task-owned work.
