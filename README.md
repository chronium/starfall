...
`Starfall.World` also owns a narrow in-process admission exchange over the existing signed-ticket Protocol facts. A running world validates a ticket against locally supplied public keys, consumes its ticket ID once, creates one generic player at the configured town respawn anchor, and binds that immutable world-local entity identity to the in-memory gameplay session. The session then accepts encoded connected-walking commands and publishes encoded snapshots or authoritative corrections without calling identity, chat, or operations. Draining rejects new joins while retaining admitted sessions, players, command handling and publication; stopping clears lifecycle-owned session, replay, walking-exchange and player state.

The current command-line host does not configure keys or expose a network transport, so its standalone runs still create only the separate technical fixture. Admission and connected-walking exchange are exercised through the bounded in-process World seam. Production key provisioning, transport security, message framing, sockets and Client transport adoption remain separate work.
